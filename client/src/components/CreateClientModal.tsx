...
const CreateClientModal: React.FC<CreateClientModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    if (!isValid) {
      ...
      logger.warn('Validation failed:', errors, 'CreateClientModal');
      return;
    }
    ...
    try {
      ...
      const clientData = {
        ...
        primaryContactTitle: sanitizedData.primaryContactTitle || null,
      };

      await apiWithTimeout(clientsAPI.create(clientData), TIMEOUT_DURATIONS.STANDARD);

      logger.log('Client created successfully', { clientName: sanitizedData.businessName }, 'CreateClientModal');
      setSuccessMessage('Client created successfully');
      setShowSuccessDialog(true);
    } catch (error: any) {
      logger.error('Client creation error:', error, 'CreateClientModal');
                  ...
                  <Card style={{ backgroundColor: currentColors.background.bg500, borderRadius: 8, elevation: 0, borderWidth: 0 }}>
                    <Card.Content style={{ paddingBottom: 0 }}>
                      <View style={styles.modalHeader}>
                        <Title style={{ color: currentColors.text, flex: 1 }}>Create New Client</Title>
                        <IconButton
